...
export default function dataReducer(state, action) {
    ...
    switch (action.type) {
        ...
        case 'MARK_FAV':
            return state
                .setIn(['entries', action.id, 'favorite'], true)
                .update('favorites_keys', (list) => list.push(action.id))
        case 'UNMARK_FAV':
            return state
                .setIn(['entries', action.id, 'favorite'], false)
                .update('favorites_keys', (list) => list.splice(list.indexOf(action.id), 1))
        case 'CREATE_ENTRY':
