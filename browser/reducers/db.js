    ...
    switch (action.type) {
        ...
        case 'DEL_FIELD':
            return state
                ...
                // delete field in user_defined
                .deleteIn(['entries', action.id, 'user_defined', action.field_id])
                //also delete the section if no more fields exist
                .updateIn(['entries', action.id, 'sections'], (list) => {
                    if (list.getIn([action.idx, 'fields']).size === 0) {
                        return list.splice(action.idx, 1)
                    } else {
                        return list
                    }
                })
