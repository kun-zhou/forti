...
function init(_vault_path, _passwd) {  // return abstracts of notes
  ...
  if (!fs.existsSync(_vault_path)) {
    return {
      success: false, message: 'VAULT_MISSING'
    }
  }
  ...
  try {
    ...
    /*
    cache is not examined here because it is regenerated if missing in
    readCache()
    */
    ...
    return {
      success: false, message: 'VAULT_CORRUPTED', error: e
    }
  }
  // get encryption key
  try {
    ...
    return {
      success: false, message: 'PASSWORD_ERROR', error: e
    }
  }
  return {
...
function cacheRefreshRead() {
  try {
    ...
    return {
      success: false, message: 'force update cache failed', error: e
    }
  }
  return {
    success: true, cache
...
function saveSecret(secret) {  // create id if not exist
  if (!secretIsValid()) {
    return {
      success: false, message: 'secret is invalid'
    }
  }
  try {
