...
const pathDefaultColorSchemes =
  path.join( appPath, 'assets', 'default_color_schemes' )
...
var defaultColorSchemes = JSON.parse( fs.readFileSync(
  path.join( pathDefaultColorSchemes, 'manifest.json' ), 'utf8' ) )
var defaultTemplates = JSON.parse(
  fs.readFileSync( path.join( pathDefaultTemplates, 'manifest.json' ), 'utf8' ) )
var config = null
...
function initialize() {
  ...
  try {
    ...
    fs.writeFileSync(
      pathConfig,
      fs.readFileSync( path.join( pathInitialization, 'config.json' ) )
    )
  ...
  // check validity of vaults (existence)
  return {
    status: 'WELCOME_BACK'
  }
}
...
function deleteVault( location ) {
  for ( var i = 0; i < config.dbs.length; i++ ) {
    if ( config.dbs[ i ].location === location ) {
      console.log( location )
      config.dbs.splice( i, 1 )
      break
    }
  }
  if ( config.last_accessed === location ) {
    config.last_accessed = ''
  }
  saveConfig()
}
...
function getColorSchemeList() {
  ...
  var all_schemes = []
  return all_schemes.push.apply( all_schemes, defaultColorSchemes )
    .push.apply( all_schemes, config.color_schemes )
}

function getColorScheme(
  scheme_file_name ) { // returns actual css of color scheme
  // Check default color schemes first
  defaultColorSchemes.forEach( ( _scheme ) => {
    if ( _scheme.file_name === scheme_file_name ) {
      return fs.readFileSync(
        path.join( pathDefaultColorSchemes, scheme_file_name ), 'utf8' )
    }
  } )
  config.color_schemes.forEach( ( _scheme ) => {
    if ( _scheme.file_name === scheme_file_name ) {
      return fs.readFileSync(
        path.join( pathColorSchemes, scheme_file_name ), 'utf8' )
    }
  } )
  return fs.readFileSync(
    path.join( pathDefaultColorSchemes, defaultColorSchemes[ 0 ].file_name ), 'utf8' )
}
...
function getCategoryIconList() {
  return new Map(
    defaultTemplates.map( ( template ) => [ template.name, template.icon ] ) )
}
  ...
  for ( var template of defaultTemplates ) {
    if ( template.name === template_name ) {
      return JSON.parse( fs.readFileSync(
        path.join( pathDefaultTemplates, template.file_name ), 'utf8' ) )
      break
    }
  }
  return null
